...
export async function getStockDetails(symbol: string) {
  ...
  const metricsUrl = `${FINNHUB_BASE_URL}/stock/metric?symbol=${encodeURIComponent(
    upperSymbol
  )}&metric=all&token=${token}`;
  try {
    const [quoteData, metricsData] = await Promise.all([
      fetchJSON<FinnhubQuote>(quoteUrl, 60),
      fetchJSON<FinnhubMetrics>(metricsUrl, 3600),
    ]);
    ...
    return {
      ...
      peRatio: metrics.peNormalized || 0,
    };
  } catch (e) {
...
export async function getStockProfile(symbol: string) {
  ...
  try {
    ...
    if (!profile || !profile.name) {
      return { name: upperSymbol, ...profile };
    }

    return profile;
  } catch (error) {
    console.error(`Failed to fetch profile for ${upperSymbol}:`, error);

    return { name: upperSymbol, ticker: upperSymbol };
  }
}
